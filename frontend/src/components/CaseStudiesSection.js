  ...
  const caseStudies = [
    {
      ...
      description:
        'Investigaciones de MIT y Stanford demostraron que sistemas comerciales de reconocimiento facial tenían tasas de error de hasta 34% más altas para mujeres de piel oscura comparado con hombres de piel clara. Empresas como Amazon, Microsoft e IBM enfrentaron críticas y algunas suspendieron temporalmente la venta de esta tecnología a policías. Este caso ilustra cómo los sesgos en datos de entrenamiento pueden perpetuar discriminación racial sistemática.',
      implications: [
    ...
    },
    {
      ...
      description:
        'Varios incidentes, incluido el accidente fatal de Uber en 2018 en Arizona, han expuesto los desafíos éticos de los vehículos autónomos. ¿Cómo debe un vehículo decidir en situaciones donde el daño es inevitable? ¿Quién es responsable cuando falla el sistema: el fabricante, el desarrollador del software o el "conductor"? El experimento "Máquina Moral" del MIT reveló diferencias culturales significativas en juicios éticos sobre estas decisiones.',
      implications: [
    ...
    },
    {
      ...
      description:
        'ProPublica reveló que COMPAS, un algoritmo usado por tribunales para predecir probabilidad de reincidencia, era significativamente más propenso a etiquetar falsamente a acusados negros como alto riesgo (45%) comparado con acusados blancos (23%). A pesar de que el algoritmo no usaba directamente la raza, variables correlacionadas resultaron en discriminación indirecta. Este caso generó debates sobre el uso de IA en decisiones judiciales.',
      implications: [
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8" data-stagger>
          {caseStudies.map((caseStudy) => (
            ...
            >
              <SafeImage src={caseStudy.image} alt={caseStudy.title} loading="lazy" className="w-full h-48 object-cover" />
              <CardHeader>
                ...
                <CardTitle className="text-xl font-serif text-slate-900">{caseStudy.title}</CardTitle>
                <CardDescription className="text-slate-600">{caseStudy.summary}</CardDescription>
              </CardHeader>
              <CardContent>
          ...
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 overflow-y-auto">
            <div className="bg-white max-w-4xl w-full max-h-[90vh] overflow-y-auto rounded-lg shadow-xl">
              <div className="sticky top-0 bg-white border-b border-slate-200 p-6 flex justify-between items-start">
                ...
                <button onClick={() => setSelectedCase(null)} className="text-slate-500 hover:text-slate-900 text-2xl leading-none" aria-label="Cerrar">
                  ×
                </button>
              </div>

              <div className="p-6 space-y-6">
                <SafeImage src={selectedCase.image} alt={selectedCase.title} loading="lazy" className="w-full h-64 object-cover" />
